...
def print_key_help() -> None:
    print("\n按键：")
    print("  1       : 开始/停止录制 left_raw")
    print("  2       : 开始/停止录制 right_raw")
...
def run(args: argparse.Namespace) -> None:
    ...
    left_dir = args.left_dir or os.path.join(args.save_root, "left_raw")
    right_dir = args.right_dir or os.path.join(args.save_root, "right_raw")
    ...
    rec_left = AsyncFFmpegRecorder("left", left_dir, "left_raw", args.left_fps, args.left_bitrate, args.encoder, args.container, ffmpeg_path, args.queue_size, args.gop, args.ffmpeg_loglevel)
    rec_right = AsyncFFmpegRecorder("right", right_dir, "right_raw", args.right_fps, args.right_bitrate, args.encoder, args.container, ffmpeg_path, args.queue_size, args.gop, args.ffmpeg_loglevel)
    ...
    try:
        ...
        while True:
            ...
            if key in (ord("q"), 27):
                ...
                start_or_stop_recorder(rec_left, left_raw)
            elif key == ord("2"):
                start_or_stop_recorder(rec_right, right_raw)
            elif key == ord("3"):
                wide, view = ensure_wide_and_view()
                ...
                if all_active:
                    ...
                    if not rec_left.active:
                        rec_left.start(left_raw)
                    if not rec_right.active:
                        rec_right.start(right_raw)
                ...
                cv2.imwrite(os.path.join(debug_dir, f"debug_left_{debug_idx:04d}_{ts}.jpg"), left_raw, [int(cv2.IMWRITE_JPEG_QUALITY), args.jpg_quality])
                cv2.imwrite(os.path.join(debug_dir, f"debug_right_{debug_idx:04d}_{ts}.jpg"), right_raw, [int(cv2.IMWRITE_JPEG_QUALITY), args.jpg_quality])
                wide, view = ensure_wide_and_view()
            ...
            now = time.time()
            if rec_left.active:
                rec_left.enqueue(left_raw, now)
            if rec_right.active:
                rec_right.enqueue(right_raw, now)
            if rec_wide.active:
                wide, view = ensure_wide_and_view()
