...
def choose_view_crop_from_director(
    ...
    """
    根据导播状态生成 1920x1080 视口左上角 crop_x / crop_y。

    横向：
        使用 director_state["current_anchor_x"] 作为镜头水平中心。
        这是你的导播算法带惯性的最终锚点。

    纵向：
        由于当前宽幅图高度约 1201，而 view_h=1080，纵向只有约 121 像素活动范围。
        默认使用 center，避免上下抖动。
        如果你希望更贴近人物底部，可以用 --crop-y-mode bottom / focus。
    """
    anchor_x = director_state.get("current_anchor_x", wide_w * 0.5)
    crop_x = int(round(anchor_x - view_w * 0.5))
...
def read_terminal_key() -> int:
    ...
    try:
        ...
        if not line:
            return 255
        print(f"[DEBUG-key] got: '{line}' ord={ord(line[0])}")
        return ord(line[0])
    except Exception as e:
        print(f"[DEBUG-key] exception: {e}")
        return 255


_DEBUG_TTY_WARNED = False

def _debug_tty_warn():
    global _DEBUG_TTY_WARNED
    if not _DEBUG_TTY_WARNED:
        _DEBUG_TTY_WARNED = True
        print("[DEBUG] stdin 不是 TTY！终端按键无法使用。")
        print(f"        sys.stdin.isatty() = {sys.stdin.isatty()}")
        print(f"        sys.stdin fileno  = {sys.stdin.fileno()}")
        print("        请在真正的终端里运行，不要通过管道/脚本/IDE 启动。")
...
class AsyncFFmpegH264Recorder:
    ...
    设计目标：
        1. 主循环负责实时运镜和显示，不直接做耗时的视频编码。
        2. 本类在后台线程中把 1920x1080 BGR 帧写入 ffmpeg。
        3. ffmpeg 使用 h264_rkmpp，尽量走 RK3588 的硬件编码。
        4. 队列满时丢旧帧，保证实时性优先，避免延迟堆积。

    为什么不用 cv2.VideoWriter：
        你之前测试过 cv2.VideoWriter 写 MP4 / MJPG AVI 时 write 很慢，
        录制后主循环掉到几帧。这里改成 ffmpeg + h264_rkmpp，并且放到后台线程。
    """
    ...
    def _build_cmd(self):
        """构造 ffmpeg 命令。输入是 stdin 中的 raw BGR24。"""
        return [
            "ffmpeg",
            "-y",
            "-hide_banner",
            ...
            "-r", str(self.fps),
            "-i", "-",
            # h264_rkmpp 通常需要 yuv420/nv12 类输入，这里让 ffmpeg 做格式转换。
            "-vf", "format=nv12",
            "-c:v", self.encoder,
            "-b:v", self.bitrate,
    ...
    def start(self, frame: np.ndarray) -> None:
        ...
        # MP4/H264 要求宽高通常为偶数。1920x1080 本身是偶数。
...
def choose_view_crop_from_director_fixed_window(
    director_state: dict,
    analysis: dict,
    wide_w: int,
    wide_h: int,
    ...
    anchor 模式：保留原来的连续 current_anchor_x 逻辑。
    """
    max_crop_x = max(0, int(wide_w - view_w))

    if window_crop_mode == "anchor":
        anchor_x = director_state.get("current_anchor_x", wide_w * 0.5)
    ...
    prev = crop_state.get("crop_x")
    if prev is None:
        crop_x_f = float(target_crop_x)
    else:
        delta = float(target_crop_x) - float(prev)
        if abs(delta) <= float(window_crop_snap_px):
            crop_x_f = float(target_crop_x)
        else:
            step = delta * float(window_crop_alpha)
            step = float(np.clip(step, -float(window_crop_max_step), float(window_crop_max_step)))
            crop_x_f = float(prev) + step

    crop_x = int(round(np.clip(crop_x_f, 0, max_crop_x)))
...
def run(args: argparse.Namespace) -> None:
    ...
    print("按键：r/Space 开始或停止录制，q/ESC 退出，s 保存当前 view，l 切换左路 overlap 屏蔽。")
    ...
    try:
        while not STOP_REQUESTED:
            ...
            crop_x, crop_y = choose_view_crop_from_director_fixed_window(
                ...
                window_crop_mode=args.window_crop_mode,
            ...
            # 录制状态只叠加在预览画面上。默认录制 clean view，不录制调试框。
            cv2.putText(
                vis,
                f"{recorder.status_text()} | r:rec/stop | s:jpg | q:quit | win:{crop_state.get('window_name')} target:{crop_state.get('target_crop_x')}",
                (20, 104),
            ...
            if recorder.is_recording:
                ...
                # 关键修复：
                # 原来的逻辑是：
                #     距离上一帧超过 1/record_fps 才提交，并把 last_time = now
                #
                # 如果主循环是 20~21 FPS，每帧约 47~50ms，
                # 而 record_fps=20 时目标间隔是 50ms，
                # 那么很多帧会因为 47ms < 50ms 被跳过，
                # 结果实际只写入约 10 FPS，视频就会变成 2 倍速。
                #
                # 现在改成“固定时间轴”：
                #     next_record_submit_t += record_interval
                #
                # 这样不会因为主循环略快于 20FPS 就隔帧录制。
            ...
            if key in (ord("q"), 27):
                ...
                if recorder.is_recording:
                    ...
                    # 默认保存 clean view；如果你加 --record-overlay，则保存带框和状态文字的 vis。
            ...
            if args.print_every > 0 and frame_idx % args.print_every == 0:
                print(
                    ...
                    f"window={crop_state.get('window_name')} target_crop={crop_state.get('target_crop_x')} "
                    f"crop=({crop_x},{crop_y}) rec={recorder.is_recording} "
...
def parse_args() -> argparse.Namespace:
    ...
    # 三段固定窗口运镜参数。
    parser.add_argument("--window-crop-mode", choices=["fixed", "anchor"], default="fixed",
                        help="fixed=固定 Left/Center/Right 三段；anchor=原连续锚点模式")
    parser.add_argument("--window-source", choices=["current", "target"], default="current",
                        help="fixed 模式下使用 current_window 更稳，target_window 更跟手")
    parser.add_argument("--window-crop-alpha", type=float, default=0.12, help="三段窗口 crop_x 平滑系数")
    parser.add_argument("--window-crop-max-step", type=float, default=24.0, help="三段窗口每帧最大移动像素")
    parser.add_argument("--window-crop-snap-px", type=float, default=2.0, help="距离目标小于该值时吸附到目标")

    # 本地录制参数。默认保存到本地目录，不保存到 TF 卡。
    parser.add_argument("--record-dir", default="/home/elf/work/basketball/camera_movement/director_videos",
                        help="本地运镜视频保存目录")
    ...
    parser.add_argument("--record-overlay", action="store_true",
                        help="打开后录制带检测框/状态文字的画面；默认录制干净的 1920x1080 view")
...
def main() -> None:
    ...
    print(f"window_crop_mode   : {args.window_crop_mode}, source={args.window_source}")
