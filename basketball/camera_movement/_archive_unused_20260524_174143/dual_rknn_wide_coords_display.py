...
def main():
    ...
    try:
        ...
        print("[信息] 开始：双路 RKNN -> 宽幅坐标 -> 宽图画框。按 q/ESC 退出，s 保存调试图。")
        ...
        while not STOP_REQUESTED:
            ...
            left_results = left_detector.detect(left_for_detect)
            right_results = right_detector.detect(right_raw)
            ...
            cv2.putText(
                vis,
                f"FPS:{fps:.1f} persons:{len(wide_dets)} infer+map+wide:{(t1-t0)*1000:.1f}ms",
                (30, 45),
