...
用途：
    ...
    生成一张完整宽幅拼接图，并保存为 JPG 到 TF 卡目录。

适用场景：
    1. 想保存当前双目拼接后的完整宽图，而不是只保存 1920x1080 裁切窗口。
...
和实时脚本的区别：
    ...
    save_wide_stitch_photo_to_tf.py：
        主要用于拍照保存完整宽幅图，优先保证保存结果完整、清晰、方便调试。
...
按键模式：
    ...
    如果你想预览后按 s 保存，可以加：
        --interactive
交互模式：按 s 保存
python3 save_wide_stitch_photo_to_tf.py \
    --left-device /dev/video41 \
    --right-device /dev/video43 \
    --map-file /home/elf/work/basketball/offline_build_stereo_rectify_maps/stereo_rectify_maps_wide_good.npz \
    --stitch-param /home/elf/work/basketball/offline_build_stereo_rectify_maps/stitch_params_good.npz \
    --save-dir /media/elf/7AC8-E830/basketball_wide_photos \
    --runtime-seam-x 150 \
    --runtime-blend-width 40 \
    --runtime-right-x-shift 30 \
    --runtime-right-y-shift -5 \
...
def save_jpg(img: np.ndarray, save_dir: str, prefix: str, quality: int) -> str:
    """
    保存 JPG 文件。
    ...
    filename = f"{prefix}_{current_timestamp()}.jpg"
...
def capture_and_save_once(
    ...
    path = save_jpg(wide, args.save_dir, prefix, args.jpg_quality)

    t2 = time.perf_counter()

    print("\n已保存宽幅 JPG:")
    print(f"  {path}")
    print(f"  image size     : {wide.shape[1]} x {wide.shape[0]}")
    print(f"  stitch time    : {(t1 - t0) * 1000.0:.1f} ms")
    print(f"  save jpg time  : {(t2 - t1) * 1000.0:.1f} ms")
...
def run_interactive(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        while True:
            ...
            if key == ord("s"):
                path = save_jpg(wide, args.save_dir, f"wide_{save_idx:04d}", args.jpg_quality)
                print(f"已保存: {path}")
                save_idx += 1
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Capture stereo cameras, stitch full wide image, and save JPG to TF card."
    )
